define([
    ...
    function (system, model, config, logger, partialMapper) {
        ...
        var getSpeakerPartials1 = function (speakersObservable, forceRemote) {

            alert('dsdsd');
            var query = EntityQuery.from('ConferenceTimeSlots')
                 .select('id, start');
            ...
            function querySucceeded(data) {
                ...
                var list = partialMapper.mapDtosToEntities(
                    manager, data.results, entityNames.conferencetimeslot, 'id');
                if (speakersObservable) {
                    speakersObservable(list);
                }
                log('Retrieved [Time slot] from remote data source',
                    data, true);
            }
        ...
        var getSpeakerPartials = function (speakersObservable, forceRemote) {
            ...
            if (!forceRemote) {                
                var p = getLocal('ConferencePersons', orderBy.speaker);
                if (p.length > 0) {
            ...
            var query = EntityQuery.from('Speaker')
        ...
        var getSessionPartials = function (sessionsObservable, forceRemote) {
            ...
            var query = EntityQuery.from('Sessions')
                .select('id, title, code, speakerId, conferenceTrackId, conferenceTimeSlotId, conferenceRoomId, level, tags,archaiveId')
                .orderBy('conferenceTimeSlotId, level, speaker.firstName');
        ...
        var primeData = function () {
            ...
            function success() {
                datacontext.lookups = {
                    conferencerooms: getLocal('ConferenceRooms', 'name', true),
                    conferencetracks: getLocal('ConferenceTracks', 'name', true),
                    conferencetimeslots: getLocal('ConferenceTimeSlots', 'start', true),
                    speakers: getLocal('ConferencePersons', orderBy.speaker, true),
                    archaives: getLocal('Archaives', 'name', true) //new
                };
        ...
        var datacontext = {
            ...
            getSpeakerPartials1: getSpeakerPartials1,
